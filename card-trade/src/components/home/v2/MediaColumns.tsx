    ...
    <section className="py-24 bg-white">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="grid gap-8 lg:grid-cols-2">
          <div className="rounded-[32px] border border-gray-200 bg-gray-50 p-8 flex flex-col gap-6">
            <div>
              <p className="text-xs uppercase tracking-[0.4em] text-gray-500">MEDIA</p>
              <h3 className="mt-3 text-2xl font-bold text-slate-900">最新の掲載実績</h3>
            </div>
            <div className="flex flex-wrap gap-3">
              {MEDIA_BADGES.map((badge) => (
                <span key={badge} className="rounded-full border border-gray-200 bg-white px-4 py-2 text-xs font-semibold tracking-[0.2em] text-gray-500">
                  {badge}
                </span>
              ))}
            </div>
            <div className="space-y-4">
              ...
              {mediaItems.map((announcement) => (
                <Link
                  ...
                  className="flex items-center justify-between rounded-2xl bg-white px-5 py-4 border border-gray-100 hover:border-blue-200 transition"
                >
                  <div>
                    <p className="text-xs uppercase tracking-[0.3em] text-gray-500">MEDIA</p>
                    <p className="text-lg font-semibold text-slate-900">{announcement.title}</p>
                  </div>
          ...
          <div className="rounded-[32px] border border-gray-200 bg-white p-8 flex flex-col gap-6 shadow-[0_30px_60px_rgba(15,23,42,0.08)]">
            <div>
              <p className="text-xs uppercase tracking-[0.4em] text-gray-500">COLUMN</p>
              <h3 className="mt-3 text-2xl font-bold text-slate-900">査定ノウハウ & 市況解説</h3>
            </div>
            <div className="space-y-4">
              {COLUMN_POSTS.map((post) => (
                <div key={post.id} className="border border-gray-100 rounded-2xl p-5 hover:border-blue-200 transition">
                  ...
                  <p className="mt-3 text-xl font-semibold text-slate-900">{post.title}</p>
                </div>
              ))}
            </div>
            <p className="text-sm text-slate-500">メディア掲載やコラムの寄稿依頼はこちらからご相談ください。</p>
            <Link href="/contact" className="self-start rounded-full border border-gray-200 px-6 py-3 text-sm font-semibold text-slate-700">
              取材・寄稿のご相談 →
            </Link>
          </div>
        </div>
